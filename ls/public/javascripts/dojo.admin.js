...
function eventos() 
{
    dojo.connect(dojo.byId("btnSalir"), "onclick", function(){
        window.location = base_url + "/application/logout";
    });
}
...
function cargarDatosTituloAdm(bloqueLogoHTML, bloqueTituloHTML) 
{
	...
	var store = new dojo.data.ItemFileReadStore({
	    url: base_url + '/objects?model=web_settings&format=json'
	});
	     	...
	     	var images='';
   	     	dojo.forEach(items, function(i)
   	 		{
   	 	    	...
   	 	    	if(store.getValue(i,"id")=="url_logo_oper"){
   	 	    		images += "<img src=\""+base_url+ "/public/" +store.getValue(i,"value")+ "\" />";
...
function cargarArbolMenu() 
{
	var store = new dojo.data.ItemFileWriteStore({
        url: base_url + '/application/modules?format=json',
        clearOnClose: true,
        identifier: 'id',
        label: 'label',
...
function cargarTabsPanelCentral(component, action)
{
    ...
    widget.set('href',base_url + '/application/tabs?p='+component+'&action='+action+id);
}
...
function eliminarRegistro(model, id) {
    ...
    dojo.xhrPost( {
        ...
        error:function(err){
        	...
        	window.location.href = base_url + '/application/login';
            return err;
        }
    });
    return res;
